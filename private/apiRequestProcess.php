...
// ---------
// 
function ciniki_wng_apiRequestProcess(&$ciniki, $tnid, &$request) {
    ...
    //
    if( isset($request['site']['settings']['site-ssl-force-api']) 
        && $request['site']['settings']['site-ssl-force-api'] == 'yes' 
        ) {
    ...
    if( isset($request['uri_split'][($request['cur_uri_pos']+2)]) ) {
        ...
        $rc = ciniki_core_loadMethod($ciniki, $pkg, $mod, 'wng', 'api');
        if( $rc['stat'] == 'ok' ) {
            $fn = $rc['function_call'];
    ...
    return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.wng.126', 'msg'=>'API Endpoint not found'));
}
?>
