...
// -------
//
function ciniki_wng_generatePage404($ciniki, $settings, $errors) {
...
//  header("HTTP/1.0 404 Not Found");
    ...
    ciniki_core_loadMethod($ciniki, 'ciniki', 'wng', 'private', 'generatePageHeader');
    $rc = ciniki_wng_generatePageHeader($ciniki, $settings, 'Page not found', array());
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $content .= $rc['content'];
    ...
    // Add the footer
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'wng', 'private', 'generatePageFooter');
    $rc = ciniki_wng_generatePageFooter($ciniki, $settings);
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $content .= $rc['content'];
